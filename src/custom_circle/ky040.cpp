...
static const s8 s_EncoderDirection[16] = 
{
	...
	-1,	// 0001: 00→01 counter-clockwise
	 ...
	 14,	// 0011: 00→11 invalid (both pins changed)
	 1,	// 0100: 01→00 clockwise
	 0,	// 0101: 01→01 no change
	 14,	// 0110: 01→10 invalid (both pins changed)
	-1,	// 0111: 01→11 counter-clockwise
	-1,	// 1000: 10→00 counter-clockwise
	 14,	// 1001: 10→01 invalid (both pins changed)
	 0,	// 1010: 10→10 no change
	 1,	// 1011: 10→11 clockwise
	 14,	// 1100: 11→00 invalid (both pins changed)
...
void CKY040::EncoderInterruptHandler (void *pParam)
{
    ...
    pThis->m_nLastCLK = ((pThis->m_nLastCLK & 0x03) << 2) | (nCLK << 1) | nDT;
    
    // lrsum = lrsum + TRANS[lrmem];
    // Používame m_nStepCounter ako tvoj lrsum
    pThis->m_nStepCounter += s_EncoderDirection[pThis->m_nLastCLK];

    TEvent Event = EventUnknown;

    // Encoder not in the neutral (detent) state
    if (pThis->m_nStepCounter % 4 != 0) return;

    // Encoder in the neutral state - clockwise rotation (lrsum == 4)
    if (pThis->m_nStepCounter == 4)
    {
        pThis->m_nStepCounter = 0;
        Event = EventClockwise;
    }
    // Encoder in the neutral state - anti-clockwise rotation (lrsum == -4)
    else if (pThis->m_nStepCounter == -4)
    {
        pThis->m_nStepCounter = 0;
    ...
    {
        // lrsum > 0 if the impossible transition (14), reset
        pThis->m_nStepCounter = 0;
    }

    // Ak sme našli platný celý krok, pošleme ho do MiniDexedu
